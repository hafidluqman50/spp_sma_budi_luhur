            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="card-box">
                        ...
                        <form action="{{ url('/admin/data-petugas/save') }}" method="POST" oninput="re_type_password.setCustomValidity(re_type_password.value != password.value ? 'Passwords do not match.' : '')">
                            @csrf
                                ...
                                <div class="col-7">
                                    <select name="jabatan_petugas" class="form-control select2">
                                        ...
                                        <option value="bendahara-internal" @if (old('jabatan_petugas') != '')
                                            {!! old('jabatan_petugas') == 'bendahara-internal' ? 'selected="selected"' : '' !!}@endif>Bendahara Internal</option>
                                        <option value="bendahara-dana-BOS" @if (old('jabatan_petugas') != '')
                                            {!! old('jabatan_petugas') == 'bendahara-dana-BOS' ? 'selected="selected"' : '' !!}@endif>Bendahara Dana BOS</option>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row">
