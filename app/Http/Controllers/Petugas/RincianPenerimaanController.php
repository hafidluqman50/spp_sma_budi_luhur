...
class RincianPenerimaanController extends Controller
{
    public function index($id)
    {
        ...
        return view('Petugas.rincian-penerimaan.main',compact('title','id','tahun_ajaran','bulan_laporan','check'));
    }
    ...
    public function tambah($id)
    {
        ...
        return view('Petugas.rincian-penerimaan.rincian-penerimaan-tambah',compact('title','id','pendapatan','tahun_ajaran','bulan_laporan','bulan','bon_pengajuan','realisasi_pengeluaran'));
    }
    ...
    public function save(Request $request,$id)
    {
        ...
        // if ($pendapatan != '') {
            for ($i=0; $i < count($pendapatan); $i++) { 
                $data_rincian_penerimaan_detail = [
                    'id_rincian_pengeluaran'         => $id,
                    'id_rincian_pengeluaran_sekolah' => $pendapatan[$i],
        ...
        return redirect('/petugas/data-perincian-rab/rincian-penerimaan/'.$id)->with('message','Berhasil Input Data');
    }

    public function edit($id)
    {
        $title        = 'Edit Rincian Penerimaan';
        $pendapatan   = RincianPengeluaranSekolah::leftJoin('kolom_spp','rincian_pengeluaran_sekolah.id_kolom_spp','=','kolom_spp.id_kolom_spp')->where('id_rincian_pengeluaran',$id)->get();
        $tahun_ajaran = RincianPengeluaran::join('tahun_ajaran','rincian_pengeluaran.id_tahun_ajaran','=','tahun_ajaran.id_tahun_ajaran')->where('id_rincian_pengeluaran',$id)->firstOrFail()->tahun_ajaran;
        ...
        $row = RincianPenerimaanDetail::where('id_rincian_pengeluaran',$id)->get();

        $row_penerimaan_rekap = RincianPenerimaanRekap::where('id_rincian_pengeluaran',$id)->firstOrFail();

        $row_penerimaan_tahun_ajaran = new RincianPenerimaanTahunAjaran;

        return view('Petugas.rincian-penerimaan.rincian-penerimaan-edit',compact('title','id','pendapatan','tahun_ajaran','bulan_laporan','bulan','bon_pengajuan','realisasi_pengeluaran','row','row_penerimaan_rekap','row_penerimaan_tahun_ajaran'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $id_rincian_penerimaan_detail       = $request->id_rincian_penerimaan_detail;
        $id_rincian_penerimaan_rekap        = $request->id_rincian_penerimaan_rekap;
        $id_rincian_penerimaan_tahun_ajaran = $request->id_rincian_penerimaan_tahun_ajaran;
        // dd($id_rincian_penerimaan_detail);
        ...
        // if ($pendapatan != '') {
            for ($i=0; $i < count($pendapatan); $i++) { 
                ...
                if ($id_rincian_penerimaan_detail[$i] == '') {
                    RincianPenerimaanDetail::create($data_rincian_penerimaan_detail);
                }
                else {
                    RincianPenerimaanDetail::where('id_rincian_penerimaan_detail',$id_rincian_penerimaan_detail[$i])->update($data_rincian_penerimaan_detail);
                }
            }
        // }
        // else {
        ...
        RincianPenerimaanRekap::where('id_rincian_penerimaan_rekap',$id_rincian_penerimaan_rekap)->update($data_rincian_penerimaan_rekap);
        ...
        for ($j=0; $j < count($bulan_rincian); $j++) {
            ...
            if ($id_rincian_penerimaan_tahun_ajaran[$j] == '') {
                RincianPenerimaanTahunAjaran::create($data_penerimaan_tahun_ajaran);
            }
            else {
                RincianPenerimaanTahunAjaran::where('id_rincian_penerimaan_tahun_ajaran',$id_rincian_penerimaan_tahun_ajaran[$j])->update($data_penerimaan_tahun_ajaran);   
            }
        }
    ...
    public function delete($id)
    {
        ...
        return redirect('/petugas/data-perincian-rab/rincian-penerimaan/'.$id)->with('message','Berhasil Hapus Data');
    }
}
